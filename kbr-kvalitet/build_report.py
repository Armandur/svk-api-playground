...
def closest_match(kbr_lat, kbr_lng, candidates, lat_k, lng_k):
    best, best_d = None, float("inf")
    for c in candidates:
        d = haversine(kbr_lat, kbr_lng, c[lat_k], c[lng_k])
        if d < best_d:
            best, best_d = c, d
    return (best, best_d) if best_d <= MAX_MATCH_M else (None, None)
...
for c in kbr_churches:
    ...
    if pm:
        row.update({"platser_id": pm["platser_id"], "platser_lat": pm["platser_lat"],
                    "platser_lng": pm["platser_lng"], "avstand_platser_m": pd})
    if om:
